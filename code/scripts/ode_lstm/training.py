...
from scripts.ode_lstm.dataset import get_loaders
from scripts.ode_lstm.ode_lstm import OdeLstm
...
def train(train_loader, model, optimizer, criterion, device, tqdm_disable=False):
    ...
    for inputs, inputs_t, targets in tqdm(train_loader, total=n_train, disable=tqdm_disable):
        inputs = inputs.to(device)
        inputs_t = inputs_t.to(device)
        targets = targets.to(device)
        outputs = model(inputs, inputs_t)

        loss_ = criterion(outputs, targets)
        loss_.backward()
        optimizer.step()
        optimizer.zero_grad()
        print(loss_.detach().cpu().item())
...
def test(test_loader, model, criterion, device):
    ...
    with torch.no_grad():
        for inputs, inputs_t, targets in test_loader:
            inputs = inputs.to(device)
            inputs_t = inputs_t.to(device)
            targets = targets.to(device)
            outputs = model(inputs, inputs_t)
...
if __name__ == '__main__':
    ...
    model = OdeLstm(in_features=INPUT_DIM, hidden_size=HIDDEN_DIM, out_feature=OUTPUT_DIM, return_sequences=False)
    model.to(device)
